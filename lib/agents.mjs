...
import { parseCostModel, estimateUsd } from './preflight.mjs';

const COMPLEXITY = { low: 0, medium: 1, high: 2 };

// Build the agent's prompt. On escalation the dispatcher passes ctx.priorFailure, so a
// retried chore gets a SHARPER prompt (the deterministic ladder) instead of a blind re-run.
function buildPrompt(task, ctx) {
  let p = `${task.title}\n\n${task.body ?? ''}`;
  if (ctx?.priorFailure) {
    p += `\n\n---\nA previous attempt by '${ctx.priorFailure.agent}' did NOT pass the check:\n`
      + `${ctx.priorFailure.detail}\n`
      + `Address that specifically and make the smallest change that makes the check pass.`;
  }
  return p;
}
...
export function makeAgent(cfg, { ledger, nightStart } = {}) {
  ...
  // Preflight cost model (V2; null unless this agent configures `cost_model` → dormant).
  const costModel = parseCostModel(o);
  const estimate = (task) => estimateUsd(task?.est_context_tokens, costModel);
  const base = { name: cfg.name, kind: cfg.kind, costRank: cfg.cost_rank ?? 0, capability: cfg.capability, estimate };
