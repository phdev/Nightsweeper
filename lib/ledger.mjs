...
export class Ledger {
  ...
  // A chore is "done" (deduped from future runs) only if it PASSED. Failed/parked chores
  // are NOT deduped — they re-enter the backlog next run instead of being silently dropped.
  hasRun(taskId) { return this._rows.some((r) => r.task_id === taskId && r.passed); }
