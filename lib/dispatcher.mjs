...
export class Dispatcher {
  ...
  _record(task, agent, result, passed, escalated, branch, reason, consumed) {
    this.ledger.record({
  ...
  async _process(task) {
    ...
    while (true) {
      ...
      if (validation.result === 'passed') {
        ...
        this._record(task, agent.name, 'passed', true, escalated, h.branch, null, result.consumedUsd ?? 0);
        return;
      }
      ...
      if (next.length) {
        this._record(task, agent.name, vres, false, escalated, null, null, result.consumedUsd ?? 0);
        this.iso.cleanup(task, false);
        escalated = true;
        continue;
      }
      ...
      this._record(task, agent.name, vres, false, escalated, null,
        escalated ? 'escalation-exhausted' : 'no-escalation-lane', result.consumedUsd ?? 0);
      return;
    }
  }
