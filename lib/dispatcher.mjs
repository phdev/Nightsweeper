  ...
  async _process(task) {
    ...
    while (true) {
      ...
      if (next.length) {
        ...
        this.iso.cleanup(task, false);
        // Deterministic escalation ladder: feed the validator's failure detail forward so the
        // next (costlier) agent gets a sharper prompt — NOT an identical blind re-run.
        task.context = { ...(task.context ?? {}), priorFailure: { agent: agent.name, detail: validation.detail || vres } };
        escalated = true;
        continue;
      }
