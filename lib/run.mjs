...
import { makeSource } from './sources.mjs';
...
export async function runNight({ configPath = 'nightsweeper.config.yaml', only } = {}) {
  ...
  let tasks = loadTasks(config.tasksFile);
  for (const s of config.sources) {
    try { tasks.push(...(await makeSource(s).fetch())); }
    catch (e) { console.error(`nightsweeper: source '${s.name}' failed: ${e.message}`); }
  }
  tasks = tasks.filter((t) => !ledger.hasRun(t.id));
